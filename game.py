...
def draw_ui():
    ...
    # UI will still render on top because it's called last in showScreen().
    ...
    if not domain_animating:
        ...
        bar_width = 200
        bar_height = 20
        draw_rect(bar_x, bar_y, bar_width, bar_height, 0.2, 0.2, 0.2) 
        health_ratio = max(0, player_health) / max_health
        draw_rect(bar_x, bar_y, bar_width * health_ratio, bar_height, 0.0, 1.0, 0.0) 
        ...
        if boss_active:
            ...
            boss_bar_y = 740
            draw_rect(boss_bar_x, boss_bar_y, boss_bar_width, 25, 0.2, 0.0, 0.0) 
            boss_health_ratio = max(0, boss_health) / boss_max_health
            draw_rect(boss_bar_x, boss_bar_y, boss_bar_width * boss_health_ratio, 25, 1.0, 0.0, 0.0) 
            
            glColor3f(1.0, 1.0, 1.0) 
            draw_text(boss_bar_x + 150, boss_bar_y + 30, f"DOMAIN BOSS", font=GLUT_BITMAP_TIMES_ROMAN_24)
    
    if game_over:
        draw_rect(0, 0, 1000, 800, 0.2, 0.0, 0.0) 
        glColor3f(1.0, 1.0, 1.0) 
        ...
        draw_rect(0, 0, 1000, 800, 0.0, 0.2, 0.0) 
        glColor3f(1.0, 1.0, 1.0) 
...
def draw_boss_shockwave():
    ...
    if boss_shockwave_active:
        ...
        # Solid quad strip for the inner band
...
def draw_player():
    ...
    for s in [-1, 1]:
        glPushMatrix()
        if domain_mode: glColor3f(0.4, 0.0, 0.6)
    ...
    if not orb_active:
        ...
        if domain_mode: glColor3f(1.0, 1.0, 1.0) 
...
def showScreen():
    ...
    # Reordered drawing sequence to simulate depth testing using the Painter's Algorithm.
    # Background and static objects are drawn first, then entities, then the UI.
    draw_ground()
    draw_scenery()
    ...
    if domain_animating:
        ...
        glutWireSphere(expansion_radius, 40, 40)
        glPopMatrix()
    ...
    # Player and projectiles drawn late to appear on top
    draw_player()
    if orb_active: draw_orb_projectile()
    if boss_orb_active: draw_boss_orb()
        
    # UI is drawn dead last to overlay the screen
    draw_ui() 
    glutSwapBuffers()
